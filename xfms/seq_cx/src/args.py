...
class BertCxArguments:
    ...
    validation_ratio: Optional[float] = field(
        default=0.2, metadata={"help": "Validation ratio (0-1)"}
    )
...
@dataclass
class BertCxConfig(BertCxArguments):
    """
    Conditional HMM training configuration
    """
    label_types: Optional[List[str]] = None

    def from_args(self, args: BertCxArguments) -> "BertCxConfig":
        """
    ...
    def save(self, file_dir: str) -> "BertCxConfig":
        """
    ...
    def load(self, file_dir: str) -> "BertCxConfig":
        """
    ...
    def from_config(self, config) -> "BertCxConfig":
        """
    ...
    def lb2idx(self):
        if self.label_types is not None:
            return {lb: idx for idx, lb in enumerate(self.label_types)}
        else:
            return {}
    ...
    def n_lbs(self):
        ...
        """
        return len(self.label_types) if self.label_types is not None else 0
