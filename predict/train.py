...
from .collate import Collator, ROPCollator
...
class Trainer(BaseTrainer, ABC):
    def __init__(self,
        ...
        if not collate_fn:
            collate_fn = Collator(task=config.task)
        ...
        self._valid_metric = 'roc_auc' if config.task == 'classification' else 'mae'
        self._status = Status(metric_smaller_is_better=True if config.task == 'regression' else False)
    ...
    def run(self):
        ...
        if self.config.model_dir \
                and os.path.exists(os.path.join(self.config.model_dir, f"{self.config.model_name}.bin")) \
                and not self.config.overwrite_output:
            self.load(self.config.model_dir, model_name=self.config.model_name)
        else:
    ...
    def evaluate(self, dataset, n_run: Optional[int] = 1, save_preds: Optional[bool] = False):
        ...
        if save_preds:
            if n_run == 1:
                file_path = os.path.join(self.config.output_dir, "preds.csv")
                save_results(lbs=dataset.lbs, preds=preds, file_path=file_path)
            else:
                for idx, pred in enumerate(preds):
                    file_path = os.path.join(self.config.output_dir, "preds", f"{idx}.csv")
                    save_results(lbs=dataset.lbs, preds=pred, file_path=file_path)

        if self.config.task == 'classification':
            metrics = get_classification_metrics(dataset.lbs, preds if n_run == 1 else preds.mean(axis=0))
        elif self.config.task == 'regression':
            metrics = get_regression_metrics(dataset.lbs, preds if n_run == 1 else preds.mean(axis=0))
        else:
    ...
    def test(self):
        ...
        if self._status.model_buffer.size == 1:
            ...
            metrics = self.evaluate(self._test_dataset, save_preds=True)
            return metrics
