...
class DNN(nn.Module):
    ...
    def __init__(self,
                 ...
                 is_rop_dataset: Optional[bool] = False):
        ...
        if is_rop_dataset:
            self.r_len_mapping_layer = nn.Sequential(
    ...
    def forward(self, batch: Batch, **kwargs):
        ...
        if getattr(self, 'r_len_mapping_layer', None) is not None:
            f_len = self.r_len_mapping_layer(batch.r_lens.unsqueeze(-1))
            x = x + f_len
